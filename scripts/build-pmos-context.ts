...
function buildMarkdown(ctx: ContextResponse): string {
  ...
  const lines: string[] = [
    ...
    `> Source: Leaxaro PMOS — /api/context/active`,
    ``,
    `---`,
    ``,
  ]
...
async function main() {
  ...
  try {
    ...
    console.error(`   Run: cd apps/pmos && npx dotenv-cli -e .env.local -- ./node_modules/.bin/next dev --port 3200`)
    process.exit(1)
  }
